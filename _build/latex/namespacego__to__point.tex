...
\begin{DoxyVerb}.. module:: go_to_point.py
    :platform: Unix
...
a new goal pose is received.\end{DoxyVerb}
...
\begin{DoxyVerb}Function to specify the state_ value and updates the current global state.

Args:
 state: new state

Updates the current global state\end{DoxyVerb}
...
Args:
 msg: Odometry message \end{DoxyVerb}
...
Args:
 des_yaw:  desired final yaw\end{DoxyVerb}
...
Args:
 des_yaw:  desired yaw\end{DoxyVerb}
...
\begin{DoxyVerb}Move the robot straight to the target

Set the linear and angular speed depending
on the distance to the goal.

Args:
 des_pos: desired (x, y) position\end{DoxyVerb}
...
Args:
 req: (x,y,theta) goal pose \end{DoxyVerb}
...
Returns:
 angle: normalized angle  \end{DoxyVerb}
